...
def insert_to_db(path):
    ...
    for i in tqdm(range(1, 34)):
        ...
        rank = sheet.cell(i, column["清朗指数"]).value
        num = sheet.cell(i, column["标准化值"]).value
        value = (city, rank, num)
        insert = "INSERT INTO public_sentiment_2020(城市,清朗指数,标准化值) VALUES(%s,%s,%s)"
...
def update(path):
    ...
    print(city_list)
    for i in range(0, 33):
        num = round((rank_list[i]-min_value)/(max_value-min_value), 9)
        city = city_list[i]
        rank = rank_list[i]
        sql = f'UPDATE public_sentiment_2020 SET 清朗指数={rank}, 标准化值={num} where 城市 = \'{city}\' '.format(num=num, rank=rank, city=city)
